...
GGobi is a tool for highly interactive and dynamic graphic analysis.  It allows you to explore your data with tools such as linked brushing and the grand tour.  Linked brushing is a key component of interactive graphics, and is necessary as soon as you have more than one plot of the same data.  Linked brushing lets you identify which points in different plots correspond to the same underlying  observation.  It has many .  The grand tour makes up the dynamic component of GGobi.  If you imagine your data as points in a $p$ dimensional space, the grand tour randomly and smoothly rotates your data and projects it down on a 2D plane.  With some training, the grand tour allows you to see high-dimensional phenomenon that you can't otherwise.  

This article does assumes some familiarity with GGobi.  If you would like find out more, or see examples of GGobi in use, please see the GGobi webpage: \url{http://www.ggobi.org}.  The website also has demos of linked brushing and the grand tour at \url{http://www.ggobi.org/docs}.  GGobi's exploratory graphics provide a natural complement to R's high quality static graphics and powerful statistical algorithms.  The {\tt rggobi} package connects R to GGobi, so that you can easily transition between analysis and exploration.  This article shows you how you can starting using {\tt rggobi} now, and demonstrates some ways to gain new insight into your data using a combination of analysis and visualisation.
...
\noindent We'll discuss each of these in turn, providing the basic syntax and some examples of what you can do. We will conclude with a case study of using {\tt rggobi} for XXX. We have tried to ensure that all the functions are well documented, so you can get more details and examples through the online help: {\tt ?ggobi}.
...
One use of GGobi is to look for multivariate outliers.  It is customary to look at uni- or bivariate plots to look for uni- or bivariate outliers, but higher dimensional outliers may go unnoticed.  Looking for these outliers is easy to do with the tour.  Change to tour view and then select all the variables.  Watch the tour and look for points that are far away from the others in some projections.

[Picture here ]

As changes to the data are updated in GGobi immediately, you can use these functions to create animations.  This example scrolls through a long time series. [Update example]
...
Adding more datasets to the GGobi instance is easy too: {\tt g\$mtcars2 <- mtcars} will add another dataset named ``mtcars2''.  You can also load any file that GGobi recognises by passing the path to the file.  In conjunction with {\tt ggobi\_find\_file}, which locates files in the GGobi installation directory, this makes it easy to load GGobi sample data.  This example loads the olives dataset included with GGobi.

\begin{alltt}
ggobi(ggobi_find_file("data", "olives.csv"))
\end{alltt}

Edge data sets are a special type of dataset in GGobi.  Instead of representing points, they represent connections, or edges, between points.  
...
A special case of data with edges is time series or longitudinal data, in which observations adjacent in time are combined 
Rggobi also provides a convenient function for creating edge sets for longitudinal data, {\tt ggobi\_longitudinal}.  This will link together observations in sequential time order and is good for looking at time series or longitudinal data.
...
For straight time series data (already in order, with no grouping variables), just use {\tt ggobi\_longitudinal} with no other arguments
...
Brushing lets you interactively change the colour, shape and size of points on plot.  Because brushing is linked, these changes will propagate to every plot where the brushed observations are displayed.  Brushing also includes shadowing, so that points sit in the background and have less visual impact, and exclusion.  You can also brush points ``automatically'', from R, using the following functions:
...
This example demonstrates the use of the some of these functions to highlight points of interest from a regression.
...
A {\tt GGobiDisplay} represents related plots in a window, and corresponds to a plot window in GGobi.  With {\tt rggobi} you can to create new displays, change the interaction or projection mode of an existing plot, or change which variables are selected. 
