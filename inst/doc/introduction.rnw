...
This example demonstrates the use of the {\tt glyph\_colour} to show the results of clustering the infamous Iris data using hierachical clustering.  The graphic shows a single projection from the grand tour.
...
\includegraphics[width=2.4in]{clustering.png}
